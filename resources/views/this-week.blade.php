...
@section('content')
    ...
    <div class="container-xl">
        ...
        <div class="page-body">
            ...
            <div class="mb-3">
                <form action="{{route('store.thisWeek')}}" method="POST"  >
                    @csrf
                    @if($errors->any())
            ...
            @php
                $todayTasks = $tasks->filter(function($task) {
                    return $task->due_date_at->isToday() && !$task->isCompleted();
                });
                $tomorrowTasks = $tasks->filter(function($task) {
                    return $task->due_date_at->isTomorrow() && !$task->isCompleted();
                });
            @endphp

            @if($todayTasks->isEmpty() )
                <div class="text-center" role="alert">
                    <strong>No Task Today</strong><br>
                    <span>Click the input box above to add a new task</span>
                </div>
            @else
                @foreach($todayTasks as $task)
                    ...
                    <span class="input-group-text">
                        <form action="{{route('task.completed',$task->id)}}" method="POST">
                            @csrf
                            <button class="form-check-input m-0" type="checkbox" style="border-color: {{$task->getPriorityColor()}}"></button>
                        </form>
                    </span>
...
{{--                    modal --}}
            ...
            @if($tomorrowTasks->isEmpty() )
                <div class="text-center" role="alert">
                    <strong>No Task Tomorrow</strong><br>
                    <span>Click the input box above to add a new task</span>
                </div>
            @else
            @foreach($tasks as $task)
                @if($task->due_date_at->isTomorrow())
            ...
            @endforeach
            @endif
            <h3> Other Tasks : 0m</h3>
            @foreach($tasks as $task)
                @if(!$task->due_date_at->isToday() && !$task->due_date_at->isTomorrow())
                    <div class="border-0 rounded-2 bg-white mb-2  @if($task->isCompleted()) d-none @endif" >
                        <div class="input-group input-group-flat " style="min-height: 46px;">
                <span class="input-group-text  ">
                    <form action="{{route('task.completed',$task->id)}}" method="POST">
